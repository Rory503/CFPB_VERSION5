        ...
        with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
            ...
            if 'CFPB_Verification_URL' in export_df.columns:
                ...
                for row in range(1, len(export_df) + 1):
                    worksheet.write_url(row, url_col, export_df.iloc[row-1, url_col], 
                                      url_format, 'Verify on CFPB.gov')
        ...
        with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
            ...
            for cat_name, cat_data in categories_to_export.items():
                ...
                if 'CFPB_Verification_URL' in cat_data.columns:
                    ...
                    for row in range(1, len(cat_data) + 1):
                        worksheet.write_url(row, url_col, cat_data.iloc[row-1, url_col], 
                                          url_format, 'Verify')
