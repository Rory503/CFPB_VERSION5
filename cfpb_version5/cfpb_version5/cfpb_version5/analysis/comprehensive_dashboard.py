...
def create_main_charts_row(data, analyzer):
    ...
    if analyzer and analyzer.filtered_df is not None:
        try:
            ...
            # Show message instead of fake trend data
            fig.add_annotation(
                text="Real Daily Trend Data<br>Available After Analysis",
                x=0.5, y=0.5,
                xref="x3", yref="y3",
                showarrow=False,
    ...
    # Chart 4: Issue Breakdown (real CFPB data)
    ...
    # Chart 5: Geographic (use real CFPB data only)
